...
class StorageInterface(ABC):
    @abstractmethod
    def upload_file(self, file_content: Union[bytes, BinaryIO], file_name: str, content_type: str) -> str:
...
class SupabaseStorage(StorageInterface):
    ...
    def upload_file(self, file_content: Union[bytes, BinaryIO], file_name: str, content_type: str) -> str:
        ...
        try:
            ...
            # Upload the file
            logger.info(f"Uploading file {file_name} to Supabase Storage as {file_path}")
...
class AWSS3Storage(StorageInterface):
    def __init__(self, bucket_name: Optional[str] = None):
        ...
        if provider_str == StorageProvider.AWS_S3:
            if not aws_access_key or not aws_secret_key or not aws_region or not self.bucket_name:
                raise ValueError(
        ...
        elif aws_access_key or aws_secret_key: # Log if keys present but S3 not selected
             logger.info("AWS keys found in environment, but STORAGE_PROVIDER is not set to 'aws_s3'. AWS S3 Storage will not be used.")
    ...
    def _ensure_bucket_exists(self):
        ...
        try:
            ...
            if error_code == '404':
                logger.info(f"Creating S3 bucket: {self.bucket_name} in region {os.getenv('AWS_REGION')}") 
                try:
                     ...
                     logger.error(f"Failed to automatically create S3 bucket {self.bucket_name}: {create_err}")
    ...
    def upload_file(self, file_content: Union[bytes, BinaryIO], file_name: str, content_type: str) -> Dict[str, Any]:
        """Upload a file to AWS S3 and return the URL"""
        if not self.s3_client:
             raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="AWS S3 Storage is not configured or enabled.")
        try:
            # Generate a unique path to avoid collisions
            unique_id = str(uuid.uuid4())
            file_path = f"{unique_id}/{file_name}"
...
class StorageFactory:
    @staticmethod
    def get_storage(provider: StorageProvider = None, bucket_name: str = None) -> StorageInterface:
        ...
        if provider is None:
            ...
            try:
                ...
                logger.warning(f"Invalid storage provider {provider_str}, using Supabase")
                provider = StorageProvider.SUPABASE
...
class StorageService:
    def __init__(self, provider: StorageProvider = None, bucket_name: str = None):
        ...
        logger.info(f"Storage service initialized with provider: {self.storage.__class__.__name__}")
    
    def upload_document(self, file_content: Union[bytes, BinaryIO], file_name: str, content_type: str) -> Dict[str, Any]:
